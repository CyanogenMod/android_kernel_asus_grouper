...
static int smb347_configure_otg(struct i2c_client *client, 
						int enableOTG, int chargeSlaves, int stopChargeSlaves)
{
	int ret = 0;
...
static void inok_isr_work_function(struct work_struct *dat)
{
	...
	if(lastOtgState>0 && lastExternalPowerState>0) {
		...
		// normally, smb347_otg_status() is called whenever the OTG adapter is 
		// pulled or plugged but here, external power was lost while the OTG 
		// adapter remained plugged we call smb347_otg_status() now, to activate 
		// self-charging of slave devices to continue host mode in std OTG mode 
		// NOTE: if we would NOT call smb347_otg_status() here, slave devices 
		// would be without power now
